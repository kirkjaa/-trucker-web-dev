...
interface EditImportRouteModalProps {
  setIsOpenEditImportRouteModal: React.Dispatch<React.SetStateAction<boolean>>;
  editImportRouteData: IResponseCheckDupRoute | null;
  handleUpdateImportRoute: (updatedRoute: IResponseCheckDupRoute) => void;
}

export default function EditImportRouteModal({
  setIsOpenEditImportRouteModal,
  editImportRouteData,
  handleUpdateImportRoute,
}: EditImportRouteModalProps) {
  // Local State
  ...
  // Use Effect
  useEffect(() => {
    if (editImportRouteData) {
      setStartLocation(
        `${editImportRouteData.origin.province}/${editImportRouteData.origin.district}`
      );
      setEndLocation(
        `${editImportRouteData.destination.province}/${editImportRouteData.destination.district}`
      );
      setUnitPrice(editImportRouteData.unit);
      setPrice(+editImportRouteData.offerPrice);
    }
  }, [editImportRouteData]);
  ...
  const handleConfirmEdit = () => {
    if (!editImportRouteData) return;

    const updatedRoute: IResponseCheckDupRoute = {
      ...editImportRouteData,
      origin: {
    ...
    handleUpdateImportRoute(updatedRoute);
  };
        ...
        <div className="flex gap-2 rounded-lg">
          <div className="flex flex-col gap-1 w-full">
            ...
            <Input
              ...
              value={editImportRouteData?.distance.value || ""}
              disabled
            />
          </div>
