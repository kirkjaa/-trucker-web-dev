...
type SystemUsersFormProps = {
  title: string;
  factoryId?: string;
  companyId?: string;
};

export default function SystemUsersForm({
  title,
  factoryId,
  companyId,
}: SystemUsersFormProps) {
  // Hook
  const {
    handleClickBackStep,
    register,
  ...
  // Use Effect
  useEffect(() => {
    if (pathName.includes(EAdminPathName.SYSTEMUSERSFACTORIES) && factoryId) {
      fetchFormById(Number(factoryId));
    }
    if (pathName.includes(EAdminPathName.SYSTEMUSERSCOMPANIES) && companyId) {
      fetchFormById(Number(companyId));
    }
  }, [factoryId, companyId]);

  useEffect(() => {
    getOptionList();
  }, [pathName, searchOption]);
      ...
      <ModalNotification
        ...
        onConfirm={() => {
          if (companyId || factoryId) {
            handleUpdate(
              pathName.includes(EAdminPathName.SYSTEMUSERSFACTORIES)
                ? factoryId!
                : companyId!
            );
          } else {
