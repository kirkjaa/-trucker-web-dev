...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("List factory system users error:", error);
    res.status(500).json({
      statusCode: 500,
      message: {
...
router.post("/", authMiddleware, uploadSingle, async (req, res) => {
  try {
    ...
    res.status(201).json({
      statusCode: 201,
      message: {
        en: "Factory system user created successfully",
        th: "สร้างผู้ใช้ระบบของโรงงานสำเร็จ",
      },
      data: { id },
    });
  } catch (error) {
    console.error("Create factory system user error:", error);
    res.status(400).json({
      statusCode: 400,
      message: {
