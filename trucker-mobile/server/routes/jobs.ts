...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const allJobs = await query.limit(Number(limit)).offset(Number(offset));
    ...
    const jobsWithRelations = await Promise.all(
      allJobs.map(async (job) => {
        ...
        const [driver] = job.driverId
          ...
          : [null];
        const stops = await db.select().from(jobStops).where(eq(jobStops.jobId, job.id)).orderBy(jobStops.sequence);
...
router.post('/:id/stops', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingStops = await db.select().from(jobStops).where(eq(jobStops.jobId, id));
