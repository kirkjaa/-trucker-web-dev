...
export const api = {
  ...
  dashboard: {
    ...
    myStats: () => request<any>("/dashboard/my-stats"),
    recentJobs: (limit?: number) => {
      const query = limit ? `?limit=${limit}` : "";
      return request<any[]>(`/dashboard/recent-jobs${query}`);
    },
  ...
  jobs: {
    ...
    myJobs: (params?: { status?: string; limit?: number; offset?: number }) => {
      const query = new URLSearchParams(params as any).toString();
      return request<any[]>(`/jobs/my-jobs${query ? `?${query}` : ""}`);
    },
    ...
    updateStatus: (id: string, status: string, progress?: number) =>
      request<any>(`/jobs/${id}/status`, {
        method: "PATCH",
        body: JSON.stringify({ status, progress }),
      }),
    delete: (id: string) => request<any>(`/jobs/${id}`, { method: "DELETE" }),
    addStop: (jobId: string, data: any) =>
      request<any>(`/jobs/${jobId}/stops`, {
        method: "POST",
        body: JSON.stringify(data),
      }),
    updateStop: (jobId: string, stopId: string, data: any) =>
      request<any>(`/jobs/${jobId}/stops/${stopId}`, {
        method: "PATCH",
        body: JSON.stringify(data),
      }),
    checkIn: (jobId: string, stopId: string) =>
      request<any>(`/jobs/${jobId}/stops/${stopId}/checkin`, { method: "POST" }),
  },
  ...
  expenses: {
    ...
    myExpenses: (params?: {
      ...
      const query = new URLSearchParams(params as any).toString();
      return request<{ data: any[]; total: number }>(
        `/expenses/my-expenses${query ? `?${query}` : ""}`
      );
    },
  ...
  revenues: {
    ...
    update: (id: string, data: any) =>
      request<any>(`/revenues/${id}`, {
        method: "PATCH",
        body: JSON.stringify(data),
      }),
    delete: (id: string) => request<any>(`/revenues/${id}`, { method: "DELETE" }),
  },
  ...
  chat: {
    ...
    sendMessage: (id: string, data: any) =>
      request<any>(`/chat/${id}/messages`, {
        method: "POST",
        body: JSON.stringify(data),
      }),
    addParticipant: (id: string, userId: string) =>
      request<any>(`/chat/${id}/participants`, {
        method: "POST",
        body: JSON.stringify({ userId }),
      }),
    delete: (id: string) => request<any>(`/chat/${id}`, { method: "DELETE" }),
  },
};
